...
use crank_lend::state::marginfi_group::MarginfiGroupImpl;
...
async fn marginfi_group_create_success() -> anyhow::Result<()> {
    ...
    let (fee_state_key, _bump) =
        Pubkey::find_program_address(&[FEE_STATE_SEED.as_bytes()], &crank_lend::ID);

    let accounts = crank_lend::accounts::MarginfiGroupInitialize {
    ...
    let init_marginfi_group_ix = Instruction {
        ...
        data: crank_lend::instruction::MarginfiGroupInitialize {}.data(),
    };
