...
use marginfi_type_crate::{
    constants::{
        ASSET_TAG_DEFAULT, ASSET_TAG_SOL,
        ASSET_TAG_STAKED, BANKRUPT_THRESHOLD, EMISSIONS_FLAG_BORROW_ACTIVE,
        EMISSIONS_FLAG_LENDING_ACTIVE, EXP_10_I80F48,
...
/// Account count per bank in remaining_accounts:
/// - Fixed: 1 (bank only)
/// - HybridDlmmKeeper: 8 (bank + 7 oracle accounts)
/// - ASSET_TAG_STAKED: 4 (bank, oracle, lst mint, lst pool)
/// - Default: 2 (bank, oracle)
pub fn get_remaining_accounts_per_bank(bank: &Bank) -> MarginfiResult<usize> {
    if bank.config.oracle_setup == OracleSetup::Fixed {
        Ok(1)
    } else if bank.config.oracle_setup == OracleSetup::HybridDlmmKeeper {
        Ok(8)
    } else {
