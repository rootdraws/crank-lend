use crate::constants::{
    DLMM_TWAP_MIN_WINDOW_SECS, METEORA_DLMM_PROGRAM_ID, MIN_PYTH_PUSH_VERIFICATION_LEVEL,
    NATIVE_STAKE_ID, PYTH_ID, SPL_SINGLE_POOL_ID, SWITCHBOARD_PULL_ID,
};
...
pub enum OraclePriceFeedAdapter {
    ...
    HybridDlmmKeeper(HybridDlmmKeeperPriceFeed),
}
...
impl OraclePriceFeedAdapter {
    ...
    pub fn try_from_bank_with_max_age<'info>(
        ...
        match bank_config.oracle_setup {
            ...
            OracleSetup::ReservedDlmmTwap => {
                panic!("standalone MeteoraDlmmTwap has been consolidated into HybridDlmmKeeper");
            }
            OracleSetup::ReservedKeeperFed => {
                panic!("standalone KeeperFed has been consolidated into HybridDlmmKeeper");
            }
            OracleSetup::HybridDlmmKeeper => {
                check!(ais.len() == 7, MarginfiError::WrongNumberOfOracleAccounts);
                ...
                Ok(OraclePriceFeedAdapter::HybridDlmmKeeper(
                    HybridDlmmKeeperPriceFeed::load_checked(
            ...
            | OracleSetup::SolendSwitchboardPull => {
                panic!("integration oracle types have been removed");
            }
        }
    }
    ...
    pub fn validate_bank_config(
        ...
        match bank_config.oracle_setup {
            ...
            OracleSetup::SwitchboardV2 => panic!("swb v2 is deprecated"),
            OracleSetup::PythPushOracle => {
                validate_pyth_push(bank_config, oracle_ais)
            }
            OracleSetup::SwitchboardPull => {
                validate_switchboard_pull(bank_config, oracle_ais)
            }
            OracleSetup::StakedWithPythPush => {
                validate_staked_pyth_push(bank_config, oracle_ais, lst_mint, stake_pool, sol_pool)
            }
            OracleSetup::Fixed => {
                check!(oracle_ais.is_empty(), MarginfiError::WrongNumberOfOracleAccounts);
                Ok(())
            }
            OracleSetup::ReservedDlmmTwap => {
                panic!("standalone MeteoraDlmmTwap has been consolidated into HybridDlmmKeeper");
            ...
            OracleSetup::HybridDlmmKeeper => {
                validate_hybrid_dlmm_keeper(bank_config, oracle_ais)
            }
            OracleSetup::KaminoPythPush
            | OracleSetup::KaminoSwitchboardPull
            | OracleSetup::DriftPythPull
...
#[inline(never)]
fn validate_pyth_push(bank_config: &BankConfig, oracle_ais: &[AccountInfo]) -> MarginfiResult {
    check!(oracle_ais.len() == 1, MarginfiError::WrongNumberOfOracleAccounts);
    require_keys_eq!(
        oracle_ais[0].key(),
        bank_config.oracle_keys[0],
        MarginfiError::WrongOracleAccountKeys
...
#[inline(never)]
fn validate_switchboard_pull(bank_config: &BankConfig, oracle_ais: &[AccountInfo]) -> MarginfiResult {
    check!(oracle_ais.len() == 1, MarginfiError::WrongNumberOfOracleAccounts);
...
fn validate_staked_pyth_push(
    ...
    if lst_mint.is_some() && stake_pool.is_some() && sol_pool.is_some() {
        check!(oracle_ais.len() == 3, MarginfiError::WrongNumberOfOracleAccounts);
        ...
        check_eq!(oracle_ais[1].key(), lst_mint, MarginfiError::StakePoolValidationFailed);
        check!(
            oracle_ais[2].owner == &NATIVE_STAKE_ID,
            MarginfiError::StakePoolValidationFailed
        );
        check_eq!(oracle_ais[2].key(), sol_pool, MarginfiError::StakePoolValidationFailed);
...
#[inline(never)]
fn validate_hybrid_dlmm_keeper(bank_config: &BankConfig, oracle_ais: &[AccountInfo]) -> MarginfiResult {
    use crate::constants::SANCTUM_STAKE_POOL_PROGRAM_ID;
    check!(oracle_ais.len() == 7, MarginfiError::WrongNumberOfOracleAccounts);
    ...
    check!(oracle_ais[0].owner == &METEORA_DLMM_PROGRAM_ID, MarginfiError::InvalidOracleSetup);
    // [1] DlmmOracle (derived from LbPair, validated in load_checked)
    check!(oracle_ais[1].owner == &METEORA_DLMM_PROGRAM_ID, MarginfiError::InvalidOracleSetup);
    // [2] ReserveX, [3] ReserveY — SPL token accounts (derived from LbPair, validated in load_checked)
    ...
    check!(oracle_ais[4].owner == &SANCTUM_STAKE_POOL_PROGRAM_ID, MarginfiError::InvalidOracleSetup);
    ...
    load_price_update_v2_checked(&oracle_ais[5])?;
    ...
    check!(oracle_ais[6].owner == &crate::ID, MarginfiError::InvalidOracleSetup);
    Ok(())
}
...
// ---------------------------------------------------------------------------
// Meteora DLMM TWAP — shared helpers used by HybridDlmmKeeper
// ---------------------------------------------------------------------------

const DLMM_LB_PAIR_DISCRIMINATOR: [u8; 8] = [33, 11, 49, 98, 181, 101, 177, 13];
const DLMM_ORACLE_DISCRIMINATOR: [u8; 8] = [139, 194, 131, 179, 140, 179, 229, 244];

const DLMM_LBPAIR_ACTIVE_ID_OFFSET: usize = 8 + 48 + 16 + 1 + 2 + 1;
const DLMM_LBPAIR_BIN_STEP_OFFSET: usize = DLMM_LBPAIR_ACTIVE_ID_OFFSET + 4;
const DLMM_LBPAIR_RESERVE_X_OFFSET: usize = 8 + 48 + 16 + 1 + 2 + 1 + 4 + 2 + 1 + 1 + 2 + 1 + 1
    + 32 + 32;
const DLMM_LBPAIR_RESERVE_Y_OFFSET: usize = DLMM_LBPAIR_RESERVE_X_OFFSET + 32;
const DLMM_LBPAIR_ORACLE_OFFSET: usize = DLMM_LBPAIR_RESERVE_Y_OFFSET + 32
    + 16 + 32 + (144 * 2) + 32;
const DLMM_ORACLE_HEADER_SIZE: usize = 8 + 24;
const DLMM_SAMPLE_SIZE: usize = 32;
...
#[inline(never)]
fn dlmm_read_sample(data: &[u8], idx: u64) -> MarginfiResult<DlmmOracleSample> {
    let offset = DLMM_ORACLE_HEADER_SIZE + (idx as usize) * DLMM_SAMPLE_SIZE;
    check!(
        data.len() >= offset + DLMM_SAMPLE_SIZE,
...
fn dlmm_compute_twap_bin(
    ...
    check!(
        data.len() >= DLMM_ORACLE_HEADER_SIZE,
        MarginfiError::InvalidOracleSetup
    );
    check!(
        data[0..8] == DLMM_ORACLE_DISCRIMINATOR,
        MarginfiError::InvalidOracleSetup
    );

    let idx = u64::from_le_bytes(data[8..16].try_into().unwrap());
    let active_size = u64::from_le_bytes(data[16..24].try_into().unwrap());
    let length = u64::from_le_bytes(data[24..32].try_into().unwrap());

    check!(active_size >= 2, MarginfiError::InvalidOracleSetup);
    check!(idx < length, MarginfiError::InvalidOracleSetup);

    let latest = dlmm_read_sample(&data, idx)?;

    let age = clock
        .unix_timestamp
        .saturating_sub(latest.last_updated_at);
    if age > max_age as i64 {
        return err!(MarginfiError::SwitchboardStalePrice);
    }

    let min_window = DLMM_TWAP_MIN_WINDOW_SECS.max(max_age as i64 / 2);
    let mut lookback_idx = if idx == 0 { active_size - 1 } else { idx - 1 };
    let mut lookback = dlmm_read_sample(&data, lookback_idx)?;
    for _ in 0..active_size.saturating_sub(1) {
        let window = latest.last_updated_at.saturating_sub(lookback.created_at);
        if window >= min_window {
            break;
        }
        lookback_idx = if lookback_idx == 0 {
            active_size - 1
        } else {
            lookback_idx - 1
        };
        ...
        lookback = dlmm_read_sample(&data, lookback_idx)?;
    }

    let time_delta = latest
        .last_updated_at
        .checked_sub(lookback.created_at)
        .ok_or_else(math_error!())?;
    check!(time_delta > 0, MarginfiError::InvalidOracleSetup);

    let cum_delta = latest
        .cumulative_active_bin_id
        .checked_sub(lookback.cumulative_active_bin_id)
        .ok_or_else(math_error!())?;

    Ok(cum_delta
        .checked_div(time_delta as i128)
        .ok_or_else(math_error!())? as i64)
}

/// price = (1 + bin_step / 10000) ^ bin_id  (square-and-multiply in I80F48)
#[inline(never)]
fn dlmm_bin_id_to_price(bin_id: i64, bin_step: u16) -> MarginfiResult<I80F48> {
    let base = I80F48::ONE
        .checked_add(
            I80F48::from_num(bin_step)
                .checked_div(I80F48::from_num(10_000u32))
                .ok_or_else(math_error!())?,
        )
        .ok_or_else(math_error!())?;
    ...
    while e > 0 {
        if e & 1 == 1 {
            result = result.checked_mul(b).ok_or_else(math_error!())?;
        }
        b = b.checked_mul(b).ok_or_else(math_error!())?;
        e >>= 1;
    }
...
/// Validate LbPair + oracle accounts, compute TWAP price in Y-token terms,
/// and return (twap_price_in_y, reserve_x_key, reserve_y_key).
#[inline(never)]
fn dlmm_load_twap_and_reserves(
    lb_pair_ai: &AccountInfo,
    oracle_ai: &AccountInfo,
    clock: &Clock,
    max_age: u64,
) -> MarginfiResult<(I80F48, Pubkey, Pubkey)> {
    check!(
        lb_pair_ai.owner == &METEORA_DLMM_PROGRAM_ID,
        MarginfiError::InvalidOracleSetup
    );
    check!(
        oracle_ai.owner == &METEORA_DLMM_PROGRAM_ID,
        MarginfiError::InvalidOracleSetup
    );

    let lb_pair_data = lb_pair_ai.try_borrow_data()?;
    check!(
        lb_pair_data.len() > DLMM_LBPAIR_ORACLE_OFFSET + 32,
        MarginfiError::InvalidOracleSetup
    );
    check!(
        lb_pair_data[0..8] == DLMM_LB_PAIR_DISCRIMINATOR,
        MarginfiError::InvalidOracleSetup
    );

    let bin_step = u16::from_le_bytes(
        lb_pair_data[DLMM_LBPAIR_BIN_STEP_OFFSET..DLMM_LBPAIR_BIN_STEP_OFFSET + 2]
            .try_into()
            .unwrap(),
    );

    let oracle_key_bytes: [u8; 32] = lb_pair_data
        [DLMM_LBPAIR_ORACLE_OFFSET..DLMM_LBPAIR_ORACLE_OFFSET + 32]
        .try_into()
        .unwrap();
    let expected_oracle_key = Pubkey::new_from_array(oracle_key_bytes);
    require_keys_eq!(
        *oracle_ai.key,
        expected_oracle_key,
        MarginfiError::WrongOracleAccountKeys
    ...
    let reserve_x_key = Pubkey::new_from_array(
        lb_pair_data[DLMM_LBPAIR_RESERVE_X_OFFSET..DLMM_LBPAIR_RESERVE_X_OFFSET + 32]
            .try_into()
            .unwrap(),
    );
    let reserve_y_key = Pubkey::new_from_array(
        lb_pair_data[DLMM_LBPAIR_RESERVE_Y_OFFSET..DLMM_LBPAIR_RESERVE_Y_OFFSET + 32]
            .try_into()
            .unwrap(),
    );
    ...
    let twap_bin = dlmm_compute_twap_bin(oracle_ai, clock, max_age)?;
    let twap_price_in_y = dlmm_bin_id_to_price(twap_bin, bin_step)?;

    Ok((twap_price_in_y, reserve_x_key, reserve_y_key))
}

/// Read the SPL token `amount` (u64 at offset 64) from a token account.
fn read_spl_token_balance(ai: &AccountInfo) -> MarginfiResult<u64> {
    let data = ai.try_borrow_data()?;
    check!(
        data.len() >= SPL_TOKEN_ACCOUNT_AMOUNT_OFFSET + 8,
        MarginfiError::InvalidOracleSetup
    );
...
/// Read the exchange rate fields from a Sanctum (or standard) SPL StakePool account.
/// Returns (total_lamports, pool_token_supply).
fn read_stake_pool_exchange_rate(ai: &AccountInfo) -> MarginfiResult<(u64, u64)> {
    use crate::constants::SANCTUM_STAKE_POOL_PROGRAM_ID;
    check!(
        ai.owner == &SANCTUM_STAKE_POOL_PROGRAM_ID,
        MarginfiError::InvalidOracleSetup
    );
    let data = ai.try_borrow_data()?;
    check!(
        data.len() >= STAKE_POOL_POOL_TOKEN_SUPPLY_OFFSET + 8,
        MarginfiError::InvalidOracleSetup
    );
...
// ---------------------------------------------------------------------------
// Hybrid DLMM + Keeper Oracle (TVL-weighted)
// ---------------------------------------------------------------------------

use crate::state::keeper_oracle::KeeperOracleState;

/// DLMM fallback confidence factor (5%). I80F48 = 0.05 * 2^48
const HYBRID_DLMM_FALLBACK_CONF_FACTOR: I80F48 = I80F48::from_bits(14073748835533);

/// Combines a Meteora DLMM TWAP oracle with a keeper-fed Pumpswap price.
///
/// oracle_keys[0] = LbPair
/// oracle_keys[1] = StakePool (Sanctum SPL)
/// oracle_keys[2] = PythSolUsd
/// oracle_keys[3] = KeeperOracle
///
/// remaining_accounts (7):
///   [0] LbPair, [1] DlmmOracle, [2] ReserveX, [3] ReserveY,
///   [4] StakePool, [5] PythSolUsd, [6] KeeperOracle
#[cfg_attr(feature = "client", derive(Clone, Debug))]
pub struct HybridDlmmKeeperPriceFeed {
    pub effective_price: I80F48,
    pub effective_confidence: I80F48,
}

impl HybridDlmmKeeperPriceFeed {
    #[inline(never)]
    pub fn load_checked<'info>(
        lb_pair_ai: &'info AccountInfo<'info>,
        dlmm_oracle_ai: &'info AccountInfo<'info>,
        reserve_x_ai: &'info AccountInfo<'info>,
        reserve_y_ai: &'info AccountInfo<'info>,
        stake_pool_ai: &'info AccountInfo<'info>,
        pyth_sol_usd_ai: &'info AccountInfo<'info>,
        keeper_ai: &'info AccountInfo<'info>,
        clock: &Clock,
        max_age: u64,
    ) -> MarginfiResult<Self> {
        // --- DLMM TWAP price (trustless) ---
        let dlmm_result = dlmm_load_twap_and_reserves(
            lb_pair_ai, dlmm_oracle_ai, clock, max_age,
        );

        // --- crankSOL/USD from stake pool + Pyth SOL/USD ---
        let (total_lamports, pool_token_supply) =
            read_stake_pool_exchange_rate(stake_pool_ai)?;
        check!(pool_token_supply > 0, MarginfiError::ZeroSupplyInStakePool);

        let sol_usd_feed =
            PythPushOraclePriceFeed::load_checked(pyth_sol_usd_ai, clock, max_age)?;
        let sol_usd_price = sol_usd_feed
            .get_price_of_type(OraclePriceType::TimeWeighted, None, u32::MAX)?;

        let cranksol_usd = sol_usd_price
            .checked_mul(I80F48::from_num(total_lamports))
            .ok_or_else(math_error!())?
            .checked_div(I80F48::from_num(pool_token_supply))
            .ok_or_else(math_error!())?;

        // --- Keeper price (Pumpswap, may be stale) ---
        let keeper_data = Self::try_load_keeper(keeper_ai, clock, max_age);

        // --- Compute DLMM price in USD + on-chain TVL ---
        let dlmm_usd_and_tvl: Option<(I80F48, I80F48)> = match dlmm_result {
            Ok((twap_price_in_y, reserve_x_key, reserve_y_key)) => {
                require_keys_eq!(
                    *reserve_x_ai.key,
                    reserve_x_key,
                ...
                let dlmm_price_usd = twap_price_in_y
                    .checked_mul(cranksol_usd)
                    .ok_or_else(math_error!())?;

                let reserve_x_amount = read_spl_token_balance(reserve_x_ai)?;
                let reserve_y_amount = read_spl_token_balance(reserve_y_ai)?;

                let tvl_x = dlmm_price_usd
                    .checked_mul(I80F48::from_num(reserve_x_amount))
                    .ok_or_else(math_error!())?;
                let tvl_y = cranksol_usd
                    .checked_mul(I80F48::from_num(reserve_y_amount))
                    .ok_or_else(math_error!())?;
                let dlmm_tvl_usd = tvl_x
                    .checked_add(tvl_y)
                    .ok_or_else(math_error!())?;

                Some((dlmm_price_usd, dlmm_tvl_usd))
            }
            Err(_) => None,
        };

        // --- TVL-weighted aggregation ---
        let (effective_price, effective_confidence) =
            match (dlmm_usd_and_tvl, &keeper_data) {
                (Some((dlmm_p, dlmm_tvl)), Some((kp, k_conf, k_tvl))) => {
                    let total_tvl = dlmm_tvl
                        .checked_add(*k_tvl)
                        .ok_or_else(math_error!())?;
                    if total_tvl <= I80F48::ZERO {
                        let avg = dlmm_p
                            .checked_add(*kp)
                            .ok_or_else(math_error!())?
                            .checked_div(I80F48::from_num(2u32))
                            .ok_or_else(math_error!())?;
                        (avg, *k_conf)
                    } else {
                        let weighted = dlmm_p
                            .checked_mul(dlmm_tvl)
                            .ok_or_else(math_error!())?
                            .checked_add(
                                kp.checked_mul(*k_tvl).ok_or_else(math_error!())?,
                            )
                            .ok_or_else(math_error!())?
                            .checked_div(total_tvl)
                            .ok_or_else(math_error!())?;
                        let spread = if dlmm_p > *kp {
                            dlmm_p.checked_sub(*kp).ok_or_else(math_error!())?
                        } else {
                            kp.checked_sub(dlmm_p).ok_or_else(math_error!())?
                        };
                        let conf = spread
                            .checked_div(I80F48::from_num(2u32))
                            .ok_or_else(math_error!())?
                ...
                (Some((dlmm_p, _)), None) => {
                    let conf = dlmm_p
                        .checked_mul(HYBRID_DLMM_FALLBACK_CONF_FACTOR)
                        .ok_or_else(math_error!())?;
                    (dlmm_p, conf)
                }
                (None, Some((kp, k_conf, _))) => (*kp, *k_conf),
                (None, None) => {
                    return err!(MarginfiError::InvalidPrice);
                }
            };
    ...
    /// Attempt to load the keeper oracle; returns None if stale or invalid.
    fn try_load_keeper(
        ai: &AccountInfo,
        clock: &Clock,
        max_age: u64,
    ) -> Option<(I80F48, I80F48, I80F48)> {
        if ai.owner != &crate::ID {
            return None;
        }
        let data = ai.try_borrow_data().ok()?;
        let expected_disc = <KeeperOracleState as anchor_lang::Discriminator>::DISCRIMINATOR;
        if data.len() < 8 || data[..8] != *expected_disc {
            return None;
        }
        let oracle: &KeeperOracleState =
            bytemuck::from_bytes(&data[8..8 + std::mem::size_of::<KeeperOracleState>()]);

        let age = clock
            .unix_timestamp
            .saturating_sub(oracle.last_updated_at);
        if age > max_age as i64 {
            return None;
        }
        ...
        let tvl: I80F48 = oracle.pumpswap_tvl_usd.into();

        if price <= I80F48::ZERO {
            return None;
        }
        Some((price, confidence, tvl))
    }
}

impl PriceAdapter for HybridDlmmKeeperPriceFeed {
    fn get_price_of_type(
        &self,
        ...
        match bias {
            None => Ok(self.effective_price),
            Some(PriceBias::Low) => Ok(self
                .effective_price
                .checked_sub(self.effective_confidence)
                .ok_or_else(math_error!())?),
            Some(PriceBias::High) => Ok(self
                .effective_price
                .checked_add(self.effective_confidence)
                .ok_or_else(math_error!())?),
        }
    }
