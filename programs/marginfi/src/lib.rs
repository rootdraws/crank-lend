...
pub mod crank_lend {
    ...
    /// Create a keeper-fed oracle account. The keeper authority can post price updates.
    /// (admin only)
    pub fn init_keeper_oracle(
        ctx: Context<InitKeeperOracle>,
        authority: Pubkey,
    ) -> MarginfiResult {
        marginfi_group::init_keeper_oracle(ctx, authority)
    }

    /// Update the price in a keeper-fed oracle. Must be signed by the oracle's authority.
    pub fn update_keeper_oracle(
        ...
        marginfi_group::update_keeper_oracle(ctx, price, confidence, pumpswap_tvl_usd)
    }
